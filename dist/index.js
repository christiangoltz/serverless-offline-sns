    ...
    ServerlessOfflineSns.prototype.createHandler = function (fn) {
        ...
        if (!this.serverless.config.skipCacheInvalidation || Array.isArray(this.serverless.config.skipCacheInvalidation)) {
            var _loop_1 = function (key) {
                ...
                if (Array.isArray(this_1.serverless.config.skipCacheInvalidation) &&
                    this_1.serverless.config.skipCacheInvalidation.find(function (pattern) { return new RegExp(pattern).test(key); })) {
                    return "continue";
                }
